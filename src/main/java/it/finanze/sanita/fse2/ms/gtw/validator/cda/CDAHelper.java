...
public class CDAHelper {
    ...
    private static CodeDTO getCode(Element element) {
        ...
        if (!isEmpty(codeSystem))
            codeDTO.setCodeSystem(codeSystem);
        if (!isEmpty(codeSystemVersion))
            codeDTO.setVersion(codeSystemVersion);
        if (!isEmpty(codeSystemName))
            codeDTO.setCodeSystemName(codeSystemName);
        if (!isEmpty(displayName))
            codeDTO.setDisplayName(displayName);
        return codeDTO;
    }
    ...
    public static SchematronValidationResultDTO validateXMLViaSchematronFull(ISchematronResource aResSCH,
            final byte[] xml) throws Exception {
        ...
        if (validST) {
            ...
            for (Object object : asserts) {
                if (object instanceof FailedAssert) {
                    ...
                    SchematronFailedAssertionDTO failedAssertion = SchematronFailedAssertionDTO.builder()
                            .location(failedAssert.getLocation()).test(failedAssert.getTest())
                            .text(failedAssert.getText().getContent().toString()).build();
                    ...
                    SchematronFailedAssertionDTO warningAssertion = SchematronFailedAssertionDTO.builder()
                            .location(warningAssert.getLocation()).test(warningAssert.getTest())
                            .text(warningAssert.getText().getContent().toString()).build();
